...
bool initial(int scale, int wscale) {
    ...
    if (SDL_Init (SDL_INIT_EVERYTHING ) < 0 ){
        ...
        window = SDL_CreateWindow ("a crossover", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, scale*wscale+1, scale*wscale+1, SDL_WINDOW_SHOWN);
        if(window == NULL) {
    ...
    while (!quit) {
        ...
        if(MenuPlay) {
            ...
            if (UP)
                ...
                // kiem tra xem moi nó co trung vi tri với con rắn không
            if (relapse == true) {
